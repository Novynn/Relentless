...
{
    QByteArray d = parent->read(32);
    if (d.size() != 32) return false;
    QByteArrayBuilder in(d);

    m_ID             = in.getString(4).toLatin1();
    m_HeaderSize	 = in.getDWord();
    m_ArchiveSize	 = in.getDWord();
    m_FormatVersion	 = in.getWord();
    m_BlockSize		 = in.getWord();
    m_HashTablePos	 = in.getDWord();
    m_BlockTablePos	 = in.getDWord();
    m_HashTableSize	 = in.getDWord();
    m_BlockTableSize = in.getDWord();
...
bool MPQArchive::load(){
    bool opened = open(ReadOnly);
    if (!opened){
        m_Error = MPQ_CANNOT_OPEN_FILE;
        return false;
	}
...
//		}
        ...
        m_Error = MPQ_INVALID_HEADER;
		return false;
	}

	m_BlockSize = 0x200 << m_Header.m_BlockSize;
    qDebug() << ("Block size is " + QString::number(m_Header.m_BlockSize) + " bzw. " + QString::number(m_BlockSize));
    ...
    //qDebug() << ("file list @block " + QString::number(pHash->m_BlockIndex));
...
bool MPQArchive::hasFile(const QString &filename){
    HashEntry *pHash = GetHashEntry(filename.toUtf8().data());
...
MPQFile* MPQArchive::getFile(const QString &filename)
{
    HashEntry *pHash = GetHashEntry(filename.toUtf8().data());
    ...
    //qDebug() << ("file @block " + QString::number(pHash->m_BlockIndex));

    return new MPQFile(this, filename.toUtf8().data(), pBlock, pHash);
}
...
MPQArchive::HashEntry *MPQArchive::GetHashEntry(const char * file, quint32 locale)
{
    ...
    //qDebug() << file << " " << index << " " << m_Header.m_HashTableSize << " " << name2;
